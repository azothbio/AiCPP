...
def get_df_trprX(dataset_path, kFold_num, kFold_idx, use_save=False):
    ...
    if all([os.path.exists(df_trX_path), os.path.exists(df_prX_path)]):
        ...
        df_trX_neg = df_row_copy(df_trX[df_trX['y']==0], 99) # MLCPP negative set * 100
...
def doit(config):
    ...
    df_trX, df_prX = get_df_trprX(dataset_path, kFold_num, kFold_idx, True)
    trX, prX = get_trprX(dataset_path, kFold_num, kFold_idx, df_trX, df_prX, True)
