...
def init_model(trX, config):
    ...
    emb_xn2 = kL.Embedding(21, 10, name='emb_xn2')(Xinp_xn)
    print(f"emb_xn1.shape : {emb_xn1.shape}")
    print(f"emb_xn2.shape : {emb_xn2.shape}")
    
    #### LSTM
    lstm_x1 = kL.Bidirectional(kL.LSTM(5, return_sequences=True, dropout=0.2, name='lstm_xn'))(emb_xn1)
    lstm_x2 = kL.Bidirectional(kL.LSTM(5, return_sequences=True, dropout=0.2, name='lstm_xc'))(emb_xn2)
    ...
    #### self_attention
    x1_A, x1_C = attN('x1', xn13, xn13, dropout=0.1)
    x2_A, x2_C = attN('x2', xn23, xn23, dropout=0.1)
    x3_A, x3_C = attN('x3', lstm_x1, lstm_x2, dropout=0.1)
    print(f"x1_A.shape : {x1_A.shape}")
    print(f"x2_A.shape : {x2_A.shape}")

    _, x4_C = attN('x4', x1_C, x2_C, dropout=0.1)
    _, x5_C = attN('x5', x2_C, x3_C, dropout=0.1)
    _, x6_C = attN('x6', x4_C, x5_C, dropout=0.1)

    #### LSTM
    lstm_x = kL.Bidirectional(kL.LSTM(3, return_sequences=True, dropout=0.1, name='lstm_x'))(x6_C)
