...
class myCallBack(keras.callbacks.Callback):
    def __init__(self, model, prX): 
        ...
        with open(File_Log, 'w') as fp:
            fp.write("#epoch,mcc,pauc,loss,acc,val_loss,val_acc\n")
...
def init_model(trX):
    ...
    """
    xc = myCnn('cnn-1', x, nf=_DIM_CNN, ks=3, st=1, ps=1, act='relu')
    print("##JMS-Chk2>xc.shape=", xc.shape)

    xA, xC = attN('att1', xc, xc)
