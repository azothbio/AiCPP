...
def init_model(trX):
    ...
    l_mha = kL.MultiHeadAttention(num_heads=3, key_dim=3, name='l_mha')
    xmha = l_mha(xemb, xemb)
    print("##JMS-Chk2>xmha.shape=", xmha.shape)     ## (9,6)

    x = kL.Concatenate(axis=-1, name='xemb_xmha')([xemb, xmha])
    print("##JMS-Chk2>x.shape=", x.shape)     ## (9,12)

    """
    x = myCnn('cnn-1', x, nf=_DIM_CNN, ks=5, st=1, ps=1, act='relu')

    x = kL.Concatenate(axis = -2, name='concat_xc')([xemb, xc2, xc3, xc4, xc5])
    return
    """

    x = kL.Flatten(name='flat_x')(x)
    x = kL.Dropout(_DROP_OUT)(x)
    print("##JMS-Chk3>x.shape=", x.shape)
