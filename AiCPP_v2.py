...
def get_df_trprX(dataset_path, kFold_num, kFold_idx, use_save=False):
    ...
    if all([os.path.exists(df_trX_path), os.path.exists(df_prX_path)]):
        ...
        df_decoy = df_decoy.sample(frac=0.1)
...
def init_model(trX, config):
    ...
    ### Concat
    #lstm_xn = kL.Flatten(lstm_xn)
    #lstm_xc = kL.Flatten(lstm_xc)
...
def doit(config):
    ...
    mod = kM.load_model(File_Model_Save_Last, custom_objects=custom_metrics)
    print("\n\n############### model-loading..##############")
    print(mod.summary())
    print("\n\n############### model-reloaded..##############")
